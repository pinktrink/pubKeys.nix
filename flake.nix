{
  inputs.keyFlake.url = "github:pinktrink/keyFlake.nix";
  outputs = { self, keyFlake, ... }: let
    inherit (keyFlake.lib) mkKeyFlake;
  in mkKeyFlake {
    freyja = {
      host.ssh = [
        "ssh-rsa  rsa age"
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIOaFB9I6cS8iWlL7IRPaynuHzrWbLS/fnXSIQ2P7ml/m ed25519 age"
      ];
      users.chloe.ssh = [ "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIBhiyE9bWby5TFXruner51NhKMQ3BPU0c038J6Aw2rD9 ed25519 git platonic home age" ];
    };
    thrud = {
      host.ssh = [
        "ssh-rsa  rsa age"
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAICeadK6jJvuzcNsBMwhVXGH+keuW9hqFHPWxsuZNNFik ed25519 age"
      ];
      users.chloe.ssh = [ "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAINMUkZZnDOPSUi014cG/6nspo1L5E6hCWx0N/swJ7fww git platonic home age" ];
    };
    amanda-rogers.users.chloe.ssh = [ "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPAK1kucW99XBPWL9kg823TjsmNZOq4FQ1RMd1bEfUuz ed25519 home age nccd" ];
    guinan = {
      host.ssh = [
        "ssh-rsa  rsa age nccd"
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFY+LZwVOUE9vl+umM1C/l7woqvB0oZGSRDse2PUV2YH ed25519 age nccd"
      ];
      users = {
        chloe.ssh = [ "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIL9Ykx7Qq53UqoFVZFMmCLiDU4qFzzGNjkyCg1bIbjQ/ ed25519 home age" ];
        guinan.ssh = [ "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIIDAwYXdxw2VEZSBpRvvi58pkXZaESTkDlg+z5nr2sw6 ed25519 home age" ];
      };
    };
    lwaxana = {
      host.ssh = [
        "ssh-rsa  rsa age nccd"
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIG/RyVYMCgNltID660aOYpcupKMaLbGRbKfi+VErt4B/ ed25519 age nccd"
      ];
      users.chloe.ssh = [ "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFgPiKbb1NfTbHNK8+XTYQ6CeAUlMcB+ukqyww1owU4s chloe@lwaxana ed25519 home age" ];
    };
    marla-aster = {
      host.ssh = [
        "ssh-rsa  rsa age nccd"
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAICWzxzTn6XrpwalI5KriObT4y1eRz7l9rMv8Ka4kX3ZX ed25519 age nccd"
      ];
      users.chloe.ssh = [ "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJmBkkqd1KRsyDVozP+SV1Wz4Jq+u/YY4EmUSF/F9Qql chloe@marla-aster ed25519 home age" ];
    };
    chloe-pixel-6.users.chloe.ssh = [ "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIC1Um3oSEEaDuMspWzAsBh64HiNWOxwonFoWPR0sFx2J" ];
  };
}
